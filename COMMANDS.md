...
_Commands to execute the code._

Thomas Stephan Juzek, Xiaoyang Ming, Jose A. Hernandez (Florida State University)

[Readme](./README.md)

Modular scaffold for **lexical-alignment study**.  
**Scope:** prompt preparation → generation (Base/Instruct) → deletion-only cleaning → POS tagging → **LAS/TPS** estimation and scoring.
...
- [6. LAS (Lexical Alignment Score)](#6-las-lexical-alignment-score)
  - [6.1 discover (estimation phase)](#61-discover-estimation-phase)
  - [6.2 eval (scoring phase)](#62-eval-scoring-phase)
    - [6.2.1 Instance-level (sequences/documents)](#621-instance-level-sequencesdocuments)
    - [6.2.2 cLAS (dataset-level)](#622-clas-dataset-level)
  - [6.3 Batch: run all families (dataset-level)](#63-batch-run-all-families-dataset-level)
  - [Notes & defaults](#notes--defaults)
- [7. Triangulate with TPS](#7-triangulate-with-tps)
  - [7.1 estimate / discover first](#71-estimate--discover-first)
  - [7.2 Scoring (TPS)](#72-scoring-tps)
    - [Single-file examples](#single-file-examples)
  - [7.3 Score all models](#73-score-all-models)
    - [7.3.1 example 1](#731-example-1)
    - [7.3.2 example 2](#732-example-2)
- [8 Parallel computing](#8-parallel-computing)
- [9 AI assistance](#9-ai-assistance)
---
...
### 1.1 Install
```bash
git clone https://github.com/fsu-nlp/lexical-alignment-shifts.git
cd lexical-alignment-shifts
...
* **LAS/TPS** estimation (**discover**) and scoring (**eval**) at word/sequence/document/corpus levels.
...
## 6. LAS (Lexical Alignment Score)

We run in two phases:

1. **discover (estimation)** -- learn word-level weights from tagged corpora
2. **eval (scoring)** -- apply weights to new CoNLL-U to obtain LAS at different levels

### 6.1 discover (estimation phase)

* Input: tagged CoNLL-U per family (Base/Instruct) + human.
* Output: per-family and aggregate word tables with LAS weights.

```bash
# Root of tagged data from §5 (transformer model recommended)
POS_ROOT="out-ig/pre/pos-tr-full"

# Discover across all families (Base + Instruct + Human)
python scripts/analyse_las.py \
  --mode discover \
  --pos-root "$POS_ROOT" \
  --out-root out-ig/pre/analyses/las-all \
  --key lemma_pos \
  --windowk 50 \
  ...
  --aggregate macro \
  --seed 42
# Optional mapping if run labels differ:
#   --variant base:_base_ --variant instruct:_instruct_
# Typical exclusions (output only / calculation):
#   --exc-stpw-outp \
#   --exc-upos-outp PUNCT SYM NUM \
#   --exc-upos-calc PUNCT SYM NUM
```

**Only instruct models (optional)**

```bash
python scripts/analyse_las.py \
  --mode discover \
  --pos-root "$POS_ROOT" \
  --out-root out/pre/analyses/las-instruct \
  --key lemma_pos \
  --windowk 50 \
  --window-policy quantile \
  --trim-common \
  --num-windows 4 \
  --file-glob "*_instruct_*.conllu" \
  --aggregate macro \
  --seed 42
```
...
```
out/pre/analyses/las-all/
  policy=quantile_trim=yes_M=4_K=50_jitter=yes/
    aggregate/las_word_ALLMODELS_macro.csv
    <family>/las_word_<family>.csv
```

---

### 6.2 eval (scoring phase)

One script; three granularities via `--level`:

* **sequences** (formerly sLAS): sentence-level
* **documents** (formerly dLAS): continuation-level
* **dataset** (cLAS): corpus-level (keep separate)

#### 6.2.1 Instance-level (sequences/documents)

Pick the weight table (family or the aggregate, as you prefer) and score any Base/Instruct file(s).

```bash
# Example: score a single Instruct file at sentence level
python scripts/analyse_las.py \
  --mode eval \
  --level sequences \
  --wtable out/pre/analyses/las-all/policy=quantile_trim=yes_M=4_K=50_jitter=yes/llama-3.1-8b/las_word_llama-3.1-8b.csv \
  --eval-conllu out/pre/pos-tr/llama-3.1-8b/2025-09-07_instruct_pubmed-halves_greedy_bf16_rev8e91.preclean.clean.conllu \
  --out-root out/pre/analyses/las-eval \
  --key lemma_pos \
  --exc-upos-calc PUNCT SYM \
  --source-tag L31-I

# Same file, document level
python scripts/analyse_las.py \
  --mode eval \
  --level documents \
  --wtable out/pre/analyses/las-all/policy=quantile_trim=yes_M=4_K=50_jitter=yes/llama-3.1-8b/las_word_llama-3.1-8b.csv \
...
* Use `--wtable .../aggregate/las_word_ALLMODELS_macro.csv` if you want a pooled scorer across families.
* `--source-tag` is a freeform label that propagates to outputs.

#### 6.2.2 cLAS (dataset-level)

Score **corpus-level** alignment against the human baseline (or any target set). Here we exclude function-heavy UPOS to focus on content words.

```bash
python scripts/analyse_las.py \
  --mode eval \
  --level dataset \
  --wtable out/pre/analyses/las-all/policy=quantile_trim=yes_M=4_K=50_jitter=yes/aggregate/las_word_ALLMODELS_macro.csv \
  --eval-conllu out/pre/pos-tr/human/human_halves.preclean.clean.conllu \
  --out-root out/pre/analyses/las-eval-human-nvaa \
  --key lemma_pos \
  --exc-upos-calc ADP AUX CCONJ DET INTJ NUM PART PRON PROPN PUNCT SCONJ SYM X \
  --lp 2.0 \
  --source-tag humannvaa
```

* `--lp` controls the Lp-norm aggregation (use 1, 1.5, 2; paper uses 2.0 unless stated).
* If you want cLAS for each model variant, pass that model’s `.conllu` instead of the human file.

---

### 6.3 Batch: run all families (dataset-level)

This scores **cLAS** for both Base/Instruct per family using the **aggregate** weight table.

```bash
set -euo pipefail

WTABLE="out-ig/pre/analyses/las-all/policy=quantile_trim=yes_M=4_K=50_jitter=yes/aggregate/las_word_ALLMODELS_macro.csv"
ROOT="out-ig/pre/pos-tr-full"
OUT="out-ig/pre/analyses/las-eval"

FAMS=(falcon3-7b gemma-3-4b llama-3.1-8b mistral-7b-v0.3 olmo2-1124-7b yi-1.5-6b)

for M in "${FAMS[@]}"; do
  I_CONLLU=( "$ROOT/$M/"*"_instruct_"*.conllu )
  B_CONLLU=( "$ROOT/$M/"*"_base_"*.conllu )
  [[ -e "${I_CONLLU[0]}" && -e "${B_CONLLU[0]}" ]] || { echo "Missing files for $M"; exit 1; }
  ...
  # Instruct
  python scripts/analyse_las.py \
    --mode eval --level dataset \
    --wtable "$WTABLE" \
    --eval-conllu "${I_CONLLU[0]}" \
    --out-root "$OUT_M" \
    --key lemma_pos \
    --lp 2.0 \
    --source-tag "${M}-I"
...
* **Roots:** This section assumes `POS_ROOT="out/pre/pos-tr"`. If you use `out-ig/pre/pos-tr-full`, set `POS_ROOT` accordingly and the rest follows.
* **Exclusions:** For discovery, excluding `PUNCT SYM NUM` is typical; for cLAS, excluding most function classes (ADP…X) focuses on content words (N/ V/ ADJ/ ADV).
* **Variants:** If your filenames don’t contain `_base_` / `_instruct_`, provide `--variant` mappings during discovery so grouping is correct.
* **Repro:** Keep `--windowk`, `--num-windows`, `--window-policy`, and `--trim-common` fixed across runs; they define the estimator.

---

## 7. Triangulate with TPS

### 7.1 estimate / discover first
...
_Parallel computing_ will require its own commands. For now, we give one example here. As we continue expanding parallel computing abilities, the need for a separate command file grows. 

### 8.1 POS tagging on multiple GPUs

```bash
#SBATCH --job-name=transformer_postag
#SBATCH -A backfill2
#SBATCH --nodes=1
#SBATCH --gres=gpu:1
#SBATCH --ntasks-per-node=1
#SBATCH --cpus-per-task=8   # parallel threads for accelerate data preprocessing(streaming JSONL, building CoNLL-U line-by-line)
#SBATCH --mem=32G
...
# Activate your venv
source ~/llms/llm-training/bin/activate

# 1) start GPU logger in background (CSV every 5s)
srun --overlap -u bash -lc \
  'nvidia-smi --query-gpu=timestamp,index,name,utilization.gpu,utilization.memory,memory.used --format=csv -l 5' \
  > "logs/gpu-%j.csv" 2>&1 &
LOGGER_PID=$!

# (optional) one-time snapshot too
srun -u nvidia-smi -L
srun -u nvidia-smi

# 2) run training
srun -u python ~/lexical-alignment-shifts/scripts/pos_tag.py \
  --model en_core_web_trf \
  --in-out-dir ~/lexical-alignment-shifts/cleaned \
  --out-root ~/lexical-alignment-shifts/pos_trf_full_2 \
  --n-process 1 \
  --batch-size 1024

# 3) stop logger cleanly
...
This README was reformatted with GPT-5 assistance.